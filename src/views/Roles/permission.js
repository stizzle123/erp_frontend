...
class Permission extends React.Component {
  ...
  parseMenu(routes, classes, allowedroutes, sub = false) {
    ...
    return routes.map((prop, key) => {
      return (
        <GridItem xs={w} sm={w} md={w} key={key}>
          <div className={classes.checkboxAndRadio}>
            <FormControlLabel
              ...
              label={prop.name}
            />
            {prop.collapse && prop.views.length > 0
              ? this.parseMenu(prop.views, classes, allowed, true)
              : ""}
            {typeof prop.actions != "undefined" && prop.actions.length > 0
              ? this.parseActions(prop.actions, prop.path, classes, allowed)
              : ""}
          </div>
        </GridItem>
      );
          ...
          <div className={classes.checkboxAndRadio}>
            <FormControlLabel
              control={
                <Checkbox
                  checked={
                    allowed.indexOf(path + "/" + prop) >= 0 ? true : false
                  }
                  tabIndex={-1}
