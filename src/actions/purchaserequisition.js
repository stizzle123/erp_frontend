...
export function fetchAllRequistion(token, callback){
    ...
    return m.makeConnection('/purchase/requisition/', m.GET).then((response) => {
...
export function findRequisitionById(token, id, callback){
    let m = new MiddleWare(token);
    return m.makeConnection('/purchase/requisition/view/'+id, m.GET).then((response) => {
        return response.json();
    }).then(        
        (responseJson)=>{
            callback(responseJson);
...
export function submitRequisition(token, data, callback){
    let m = new MiddleWare(token);
    return m.makeConnection('/purchase/requisition/submit', m.POST, data).then(
        (result)=>{
...
export function editRequisition(token, id, data, callback){
    let m = new MiddleWare(token);
    return m.makeConnection('/purchase/requisition/update/'+id, m.POST, data).then((result)=>{
        {{debugger}}
        if(result.ok && result.statusText == "OK" && result.status == 200 )callback(result.ok);
    }); 
}
