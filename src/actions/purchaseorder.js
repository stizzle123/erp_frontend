...
export function fetchAllPurchaseOrder(token, callback){
    ...
    return m.makeConnection('/purchase/order/', m.GET).then((response) => {
...
export function submitPO(token, data, callback){
    let m = new MiddleWare(token);
    return m.makeConnection('/purchase/order/submit', m.POST, data).then(
        (result)=>{
...
export function fetchPurchaseOrderById(token, id, callback){
    let m = new MiddleWare(token);
    return m.makeConnection('/purchase/order/view/'+id, m.GET).then(
        (response)=>{
            return response.json()
        }).then(        
            (responseJson)=>{
